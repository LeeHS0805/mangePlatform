        ...
        <div class="el-head-container">
            <el-header>
                ...
                <div class="header_info">
                    ...
                    <span>{{username}}登录时长: {{loginTime| fomateLoginTime}}</span>
                </div>
...
<script>
    ...
    export default {
        ...
        filters: {
            ...
            fomateLoginTime(value) {
                var date = new Date();
                var hours = padaDate(date.getHours());
                var minutes = date.getMinutes();
                var seconds = date.getSeconds();
                var valHours = hours - value.getHours()
                var valMin = minutes - value.getMinutes()
                var valSec = seconds - value.getSeconds()
                if (valMin < 0) {
                    valMin = 60 + valMin
                    valHours--;
                }
                ...
                return valHours + '时' + valMin + '分' + valSec + '秒';
            }
        },
        watch: {
            $route(to, from) {
